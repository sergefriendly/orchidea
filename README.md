...
В данном случае метод Эйлера для численного решения дифференциальных уравнений склонен накапливать погрешность, что мы и видим на этом последнем графике — что есть не истинный результат, как мы увидим, решая систему, например, методом Рунге-Кутта 4-го порядка.
